...
func TestStatsStore_ActiveStreams(t *testing.T) {
	t.Run("iterates all streams", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			...
			require.NoError(t, s.Update(fmt.Sprintf("tenant%d", i), uint64(i), 100, clock.Now()))
		}
	...
	t.Run("does not iterate expired streams", func(t *testing.T) {
		...
		clock := quartz.NewMock(t)
		s.clock = clock
		require.NoError(t, s.Update("tenant1", 0x1, 100, clock.Now()))
...
func TestStatsStore_TenantActiveStreams(t *testing.T) {
	t.Run("iterates all streams for tenant", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			...
			// Create 10 streams, one stream for each of the 10 partitions.
			require.NoError(t, s.Update(tenant, uint64(i), 100, clock.Now()))
		}
		// Check we can iterate the streams for each tenant.
		expected1 := []uint64{0x0, 0x1, 0x2, 0x3, 0x4}
		actual1 := make([]uint64, 0, 5)
		s.WithTenantRead("tenant1", func(ts *tenantStatsReadStore) {
			for _, stream := range ts.ActiveStreams() {
		...
		s.WithTenantRead("tenant2", func(ts *tenantStatsReadStore) {
			for _, stream := range ts.ActiveStreams() {
...
func TestStatsStore_Update(t *testing.T) {
	...
	require.EqualError(t, s.Update("tenant", 0x1, 100, time1), "outside active time window")
	// Metadata within the active time window is accepted.
	time2 := clock.Now()
	require.NoError(t, s.Update("tenant", 0x1, 100, time2))
}
...
func TestStatsStore_UpdateRateBuckets(t *testing.T) {
	...
	time1 := clock.Now()
	require.NoError(t, s.Update("tenant", 0x1, 100, time1))
	...
	time3 := clock.Now()
	require.NoError(t, s.Update("tenant", 0x1, 100, time3))
	stream, ok = s.getForTests("tenant", 0x1)
	require.True(t, ok)
	...
	time4 := clock.Now()
	require.NoError(t, s.Update("tenant", 0x1, 100, time4))
	stream, ok = s.getForTests("tenant", 0x1)
	require.True(t, ok)
	...
	clock.Advance(time.Minute)
	time5 := clock.Now()
	require.NoError(t, s.Update("tenant", 0x1, 100, time5))
	stream, ok = s.getForTests("tenant", 0x1)
	require.True(t, ok)
...
func TestStatsStore_Evict(t *testing.T) {
	...
	require.NoError(t, s.Update("tenant1", 0x1, 0, clock.Now()))
	// The streams 0x2 and 0x3 should not be evicted as will still be within
	// the active time window after advancing the clock.
	require.NoError(t, s.Update("tenant2", 0x1, 0, clock.Now().Add(5*time.Minute)))
	require.NoError(t, s.Update("tenant2", 0x1, 0, clock.Now().Add(15*time.Minute)))
